...
(async()=>{for(const standalone of [false,true]){
 ...
 assert.equal(t.screen,'title');press('Enter');assert.equal(t.screen,'select');assert(t.selecting);t.update(10);assert.equal(t.countdown,3);press('KeyA');assert.equal(t.actor.state,'idle');get('playerCharacter').events.change({target:{value:'mascot'}});get('enemyCharacter').events.change({target:{value:'green'}});get('difficulty').events.change({target:{value:'hard'}});assert(t.selecting);assert.equal(t.actor.appearance,'mascot');assert.equal(t.enemy.appearance,'green');assert.equal(t.enemyAI.difficulty,'hard');get('startMatch').onclick();assert.equal(t.selecting,false);assert.equal(t.actor.appearance,'mascot');assert.equal(t.enemy.appearance,'green');t.showCharacterSelect();assert(t.selecting);press('Enter');assert.equal(t.selecting,false);
