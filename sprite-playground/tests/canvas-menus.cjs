...
(async()=>{
 ...
 try{
  ...
  const localPage=await browser.newPage();await localPage.goto('file://'+path.join(root,'index.html'));await localPage.waitForSelector('#local-start-help');assert(await localPage.locator('main').isHidden());await localPage.close();
  for(const standalone of [false,true]){
   ...
   assert.equal(await state(),'title');await page.screenshot({path:path.join(output,(standalone?'standalone-':'source-')+'title.png')});
   await click('start');assert.equal(await state(),'select');await click('fighter:blackcat');await click('enemy');await click('fighter:gray');await click('difficulty');assert.deepEqual(await page.evaluate(()=>[motionTest.actor.appearance,motionTest.enemy.appearance,motionTest.enemyAI.difficulty]),['blackcat','gray','hard']);
   await page.keyboard.press('q');await page.keyboard.press('1');await page.keyboard.press('ArrowLeft');assert.equal(await page.evaluate(()=>motionTest.actor.appearance),'fish');await page.screenshot({path:path.join(output,(standalone?'standalone-':'source-')+'select.png')});
   ...
   await click('title');assert.equal(await state(),'title');await page.setViewportSize({width:1200,height:500});await page.locator('#game').focus();await page.keyboard.press('ArrowDown');assert.equal(await page.evaluate(()=>scrollY),0,'down arrow must not scroll while the game canvas is focused');await page.setViewportSize({width:390,height:760});assert(await page.evaluate(()=>getComputedStyle(document.body,'::before').content.includes('PC専用')));assert(await page.locator('main').isHidden());
   assert.deepEqual(errors,[]);await page.close();console.log('PASS: '+(standalone?'standalone':'source')+' PC title, selection, battle, result and mobile notice');
  }
