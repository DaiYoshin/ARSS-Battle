...
(async()=>{
 ...
 try{
  ...
  for(const standalone of [false,true]){
   ...
   await click('arcade');assert.equal(await state(),'select');await click('fighter:fish');assert.notEqual(await page.evaluate(()=>motionTest.enemy.appearance),'fish');await click('arcadeStart');assert.equal(await state(),'battle');await page.screenshot({path:path.join(output,(standalone?'standalone-':'source-')+'arcade-1.png')});
   for(let match=0;match<3;match++){await page.evaluate(()=>{const t=motionTest;t.enemyAI.cooldown=1000;for(let round=0;round<2;round++){t.advanceFrame(3);t.enemy.knockDown();t.advanceFrame(0);t.advanceFrame(4);}});assert.equal(await state(),match===2?'result':'battle');}
   assert.equal(await page.locator('#matchOutcome').textContent(),'ARCADE CLEAR');assert.equal(await page.locator('#matchScore').textContent(),'3 / 3');await page.screenshot({path:path.join(output,(standalone?'standalone-':'source-')+'arcade-clear.png')});await click('title');assert.equal(await state(),'title');
   await click('arcade');await click('arcadeStart');await page.evaluate(()=>{const t=motionTest;t.enemyAI.cooldown=1000;for(let round=0;round<2;round++){t.advanceFrame(3);t.actor.knockDown();t.advanceFrame(0);t.advanceFrame(4);}});assert.equal(await state(),'result');assert.equal(await page.locator('#matchOutcome').textContent(),'ARCADE OVER');assert.equal(await page.locator('#matchScore').textContent(),'0 / 3');await click('title');
