...
(async()=>{
 ...
 try{
  ...
  for(const standalone of [false,true]){
   ...
   await click('title');assert.equal(await state(),'title');await page.setViewportSize({width:390,height:760});assert(await page.evaluate(()=>getComputedStyle(document.body,'::before').content.includes('PC専用')));assert(await page.locator('main').isHidden());
