...
assets={f.stem:'data:image/png;base64,'+base64.b64encode(f.read_bytes()).decode() for f in (p/'assets').glob('*.png')}
js='const embeddedAssets='+json.dumps(assets)+';\n'+(p/'app.js').read_text().replace("im.src='assets/'+name+'.png';","im.src=embeddedAssets[name];")
