...
html=(p/'index.html').read_text().replace('<link rel="stylesheet" href="style.css?v=button-feedback-v1">','<style>'+(p/'style.css').read_text()+'</style>').replace('<script src="character.js?v=button-feedback-v1"></script>','<script>'+(p/'character.js').read_text()+'</script>')
...
(p.parent/'sprite-playground.html').write_text(html.replace('<script src="app.js?v=button-feedback-v1"></script>','<script>'+js+'</script>'))
